...
import { buildStoleshnicyLandingSchemas } from "@/lib/schema/stoleshnicy-landing";
import { SITE } from "@/lib/site";

const INTERNAL_LINK_IMAGES: Record<string, string> = {
  "/catalog/stoly": "/category-stoly.webp",
  "/catalog/stoly/kuhonnyj": "/category-stoly.webp",
  "/catalog/stoly/s-cvetami": "/category-bukety.webp",
  "/sohranenie-buketa-v-smole": "/category-bukety.webp",
...
function ApplicationSection({
  ...
  section: (typeof STOLESHNICY_LANDING.sections)[number];
  reversed: boolean;
}) {
  return (
    <article
      id={section.id}
      className="scroll-mt-28 grid gap-8 lg:grid-cols-2 lg:items-center lg:gap-12"
    >
      <div className={reversed ? "lg:order-2" : ""}>
        <h2 className="font-serif text-2xl font-semibold leading-snug text-green sm:text-3xl">
          {section.h2}
        </h2>
        <p className="mt-4 text-sm leading-relaxed text-fg/75 sm:text-base">
          {section.text}
        </p>
        {section.link && section.linkLabel ? (
          <p className="mt-5">
            <Link
              href={section.link}
              className="text-sm font-medium text-green/80 hover:text-green hover:underline"
            >
              {section.linkLabel} →
            </Link>
          </p>
      ...
      <div className={reversed ? "lg:order-1" : ""}>
        <div className="relative aspect-[16/10] overflow-hidden rounded-[var(--radius-lg)] border border-green/12 shadow-[var(--shadow-sm)]">
          <Image
            src={section.image}
            alt={section.h2}
            fill
...
function ProcessTimeline({
  ...
  return (
    <ol className="relative mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4 lg:gap-4">
      <div
        className="absolute left-5 top-0 hidden h-full w-px bg-gradient-to-b from-green/25 via-green/15 to-transparent sm:hidden"
        aria-hidden
      />
      <div
...
export function StoleshnicyLandingView() {
  const page = STOLESHNICY_LANDING;
  const heroWhatsapp = stoleshnicyWhatsappUrl(page.hero.whatsappPrefill);
  const finalWhatsapp = stoleshnicyWhatsappUrl(page.ctaFinal.whatsappPrefill);
  const [featuredGallery, ...galleryRest] = page.gallery;

  return (
    <LandingShell>
      <JsonLd data={buildStoleshnicyLandingSchemas()} />
      ...
      <section className="relative overflow-hidden border-b border-green/10">
        ...
        />
        <div
          className="pointer-events-none absolute -left-16 top-12 h-64 w-64 rounded-full bg-sage/20 blur-3xl"
          aria-hidden
        />
        <Container className="relative py-12 lg:py-20">
          <nav className="text-sm text-fg/60" aria-label="Хлебные крошки">
            ...
            <span className="text-fg/70">Столешницы из эпоксидной смолы</span>
          </nav>

          <div className="mt-8 grid gap-10 lg:mt-12 lg:grid-cols-12 lg:items-center lg:gap-14">
            <div className="order-2 lg:order-1 lg:col-span-5">
              <SectionEyebrow>
                Замер · изготовление · монтаж · Краснодар
              </SectionEyebrow>
              ...
              <p className="mt-6 flex items-start gap-2 text-sm leading-relaxed text-fg/60">
                ...
                Стол — отдельная мебель; столешница — поверхность на гарнитур
                или тумбу
              </p>
            </div>
              ...
              <div className="relative">
                ...
                <div className="relative aspect-[5/4] overflow-hidden rounded-[var(--radius-lg)] border border-green/12 shadow-[var(--shadow)] sm:aspect-[4/3]">
                  ...
                  <p className="absolute bottom-4 left-4 right-4 max-w-sm rounded-[var(--radius)] bg-white/90 px-4 py-3 text-sm leading-snug text-green-deep backdrop-blur-sm">
                    Слэб, «река» и оттенок смолы — под ваш интерьер
                  </p>
                </div>
              </div>
            </div>
      ...
      <section className="border-t border-green/10 bg-sage-muted/20 py-12 lg:py-16">
        <Container>
          <SectionEyebrow>Где применяем</SectionEyebrow>
          <h2 className="mt-3 font-serif text-2xl font-semibold text-green sm:text-3xl">
            Столешницы под вашу зону
          </h2>
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-fg/65 sm:text-base">
            Кухня, ванная, бар или слэб с живым краем — каждый формат со своими
            нюансами монтажа и ухода.
          </p>
      ...
      {/* Price */}
      <section className="border-t border-green/10 bg-cream/30 py-12 lg:py-16">
        <Container>
            ...
            <div className="grid gap-px bg-green/10 sm:grid-cols-2">
              <div className="bg-white px-6 py-7 sm:px-8">
                ...
                <p className="mt-2 text-sm leading-relaxed text-fg/65">
                  За пог. м или изделие — по сложности заливки и породе
                </p>
              </div>
              <div className="bg-white px-6 py-7 sm:px-8">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-green/55">
                ...
                <p className="mt-2 text-sm leading-relaxed text-fg/65">
                  Согласование слэба и заливка занимают больше всего времени
                </p>
              </div>
            </div>
            <div className="border-t border-green/10 bg-gradient-to-r from-sage-muted/40 via-cream to-white px-6 py-5 sm:px-10">
      ...
      <section className="py-12 lg:py-16">
        <Container>
          ...
          <p className="mt-2 max-w-2xl text-sm text-fg/65">
            Плейсхолдеры — замените на фото реальных объектов с замера.
          </p>
          ...
          <ul className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 lg:grid-rows-2">
            ...
            {galleryRest.map((item) => (
              <li key={item.caption}>
                <article className="group h-full overflow-hidden rounded-[var(--radius-lg)] border border-green/10 bg-white shadow-[var(--shadow-sm)] transition hover:border-green/20">
      ...
      <section
        ...
        aria-labelledby="stoleshnicy-faq"
      >
        <Container>
        ...
        <Container className="relative py-12 lg:py-16">
          <div className="mx-auto max-w-2xl text-center text-white">
            ...
            <p className="mt-2 text-xs uppercase tracking-[0.18em] text-white/60">
              Замер · смета · монтаж · {SITE.phoneDisplay}
            </p>
              ...
              <li key={link.href}>
                ...
                >
                  <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-[14px] bg-sage-muted/40">
                    <Image
                      src={
                        INTERNAL_LINK_IMAGES[link.href] ?? "/category-stoly.webp"
                      }
                      alt=""
                      fill
